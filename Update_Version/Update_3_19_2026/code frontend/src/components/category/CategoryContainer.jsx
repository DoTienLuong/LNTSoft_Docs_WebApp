...
export default function CategoryContainer({ moduleId, onSelectCategory, selectedCategoryId, nameModuleSelected, iconModuleSelected, userRole, titleCategorySelected, collapsed = false }) {
  ...
  useEffect(() => {
    ...
    const current = categories.find((c) => Number(c.id) === idNum);
    if (current) {
      setSelectedCategoryType(normalizeCategoryType(current.category_type));
    }
  ...
  const scopedCategories = categories.filter(
    (c) => normalizeCategoryType(c.category_type) === selectedCategoryType
  );

  useEffect(() => {
    setOpenParents({});
    initialOpenSet.current = false;
  }, [selectedCategoryType, moduleId]);

  // ✅ Cập nhật tree mỗi lần có data mới
  const tree = buildCategoryTree(scopedCategories);
  ...
  return (
    <aside className={styles.listStyle}>
      ...
      <div className="flex-1 overflow-y-auto mt-2 pr-1 min-h-0">
        <CategoryList
          ...
          onSelectCategory={(id) => {
            ...
            const found = scopedCategories.find((c) => Number(c.id) === Number(id));
            onSelectCategory?.(id);
      ...
      <div className="shrink-0 mt-2 pt-2 bg-transparent">
        <div className="grid grid-cols-3 gap-1">
          {["Work Area", "Setting", "Report"].map((type) => {
            const active = selectedCategoryType === type;
            return (
              <button
                key={type}
                type="button"
                ...
                className={`px-2 py-1.5 text-xs font-medium rounded transition ${
                  active
                    ? "bg-slate-800 text-white dark:bg-slate-900 dark:text-slate-100"
                    : "bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-200 dark:text-slate-800 dark:hover:bg-slate-300"
                }`}
              >
                {type}
              </button>
